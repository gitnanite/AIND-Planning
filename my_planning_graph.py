...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        #Go through each valid ground actions and calculated no-op action
        action_nodes = set()
        for action in self.all_actions:
            #Create a PgNode_a object for current action
            action_node = PgNode_a(action)
            #Does this PgNode_a have prenodes that are a subset of the previous S level?
            if action_node.prenodes <= self.s_levels[level]:
			    #Add this node to a set of PgNode_a
                action_nodes.add(action_node)
                #connect this node to the S node istances in the s_level set
                for state_node in self.s_levels[level]:
                    state_node.children.add(action_node)
                    action_node.parents.add(state_node)
        #Add the set of PgNode_a to the list of sets of PgNode_a 
    ...
    def add_literal_level(self, level):
        ...
        state_nodes = set()
        #Find the A nodes in the previous level
        for action_node in self.a_levels[level-1]:
            #For each a_node found get the list of S nodes in effnodes
            for state_node in action_node.effnodes:
                #Add each S node found to the set
                state_nodes.add(state_node)
			    #connect this S node as a child of all the A nodes that could produce it
                state_node.parents.add(action_node)
				#add the A node to the paretn set of the S node
                action_node.children.add(state_node)
        #Add the set of S nodes to the list of sets of S nodes
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if (set(node_a1.action.effect_add).intersection(node_a2.action.effect_rem) or 
                set(node_a2.action.effect_add).intersection(node_a1.action.effect_rem)) :
            return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if (set(node_a1.action.effect_add).intersection(node_a2.action.precond_neg) or 
                set(node_a2.action.effect_add).intersection(node_a1.action.precond_neg) or 
                set(node_a1.action.effect_rem).intersection(node_a2.action.precond_pos) or
                set(node_a2.action.effect_rem).intersection(node_a1.action.precond_pos)):
            return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for parent_node1 in node_a1.parents:
            for parent_node2 in node_a2.parents:
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for parent_node1 in node_s1.parents:
            for parent_node2 in node_s2.parents:
                if not parent_node1.is_mutex(parent_node2):
                    return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        level_sum = 0
        goals_counted = set()
        # how many S-levels in the planning graph?
        ...
        for goal in goals:
            ...
            for level in range(levels):
                for node_s in self.s_levels[level]:
                    if node_s.literal == goal and goal not in goals_counted:
                        level_sum = level_sum + level
                        goals_counted.add(goal)
        return level_sum
